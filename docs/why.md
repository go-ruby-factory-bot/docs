...
`go-ruby-factory-bot/factory-bot` reimplements Ruby's `factory-bot` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-factory-bot/factory-bot` directly, with no Ruby runtime;
...
Because `factory-bot` is pure Go with cgo disabled, it also compiles to **WebAssembly**
